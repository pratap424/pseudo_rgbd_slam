...
class DepthQualityTests(unittest.TestCase):
    def test_compute_depth_metrics_perfect_prediction(self):
        gt = np.full((20, 20), DEPTH_1M, dtype=np.uint16)  # 1.0m for factor=5000
        pred = np.full((20, 20), DEPTH_1M, dtype=np.uint16)
    ...
    def test_compute_depth_metrics_known_values(self):
        gt = np.full((20, 20), DEPTH_1M, dtype=np.uint16)      # 1.0m
        pred = np.full((20, 20), DEPTH_1_5M, dtype=np.uint16)  # 1.5m
    ...
    def test_create_error_heatmap_marks_invalid_pixels_black(self):
        pred = np.array([[0, DEPTH_1M], [2 * DEPTH_1M, 2 * DEPTH_1M]], dtype=np.uint16)
        gt = np.array([[DEPTH_1M, DEPTH_1M], [0, 2 * DEPTH_1M]], dtype=np.uint16)
    ...
    def test_create_comparison_image_output_shape(self):
        ...
        gt_depth = np.full((4, 5), DEPTH_1M, dtype=np.uint16)
        pred_depth = np.full((4, 5), DEPTH_1M, dtype=np.uint16)
    ...
    def test_evaluate_dataset_writes_summary_and_csv(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            gt_depth = np.full((20, 20), DEPTH_1M, dtype=np.uint16)
            pred_depth = np.full((20, 20), DEPTH_1M, dtype=np.uint16)
